...
function Config() {
  ...
  useEffect(() => {
    ...
    fetch("auriadb.netlify.app/api/users/list") // dev: http://localhost:3000/api/users/list
      .then((res) => res.json())
      .then((data) => {
...
if (usuarios.length > 0) {
  ...
  const handleDelete = async (id, name) => {
      ...
      try {
        const response = await fetch(`auriadb.netlify.app/api/users/${id}`, { // dev: http://localhost:3000/api/users/${id}
          method: "DELETE",
          headers: {
